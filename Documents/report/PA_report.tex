...
\textbf{Have to:} You are now in your browser and have open the source file \textit{inphinity/show\_phamily.ipynb}. The class \textit{sea\_inphinity()} have every methods that we use in this part. cf. file if you want to see it all.

First we have to create an Inphinity() object capable to access our data. We will use a modified database 'sea\_own', we will discuss his creation later in the document (cf section 4.2.2).
...
Now we want to be able to get a philogenetic tree for a phage. To do so we choose a phame, here pham '2799' and we call our method \textit{inphinity.build\_tree('2799')}.
...
Now we are ready to use MUSCLE to aline genes together. \todo explain MUSCLE. In the future it will be interesting to take some time to customized with options our call to MUSCLE \cite{ref5}
...
Now we have our alignments ready to compute a phylogenetic tree. To realise the tree we use \textit{FastTree} software. It will produce "approximately-maximum-likelihood phylogenetic trees" using our .aln file proviously generated. 
...
In this chapter we'ill take about the results produce during this project.
...
For pham \textit{2799} we can split the pham into parts using those distances. On figure 3.20 those part are colored differentlly depending on ther appartenence.

Using a figure tree is not really practicle to make things automatic. So funtion \textit{prepare\_tree\_fig} set the attribute \textit{self.leaves}. This attribute store all gene composing a pham. Using the code from figure 3.21 it show for all gene, the phage and his host strain.
...
\textbf{Note:} See issue at section 5.2.3 about phage host strain in the current database.


\section{Database}
\subsection{SEA}
For this phase of the project have used the \textit{SEA} database \cite{ref7}. This db is populated with 113 phages and 2771 phams.

\textbf{Note:} See in annexes, file \textit{SEA.sql} for the db sql dump.

We've used PhamDb to add some more phage to the database. For now the new database, \textit{SEA\_own}, contains 167 phages and 7549 phams. An issue with PhamDb, cf. section 5.1.3, is responsable for the fact that we only add 84 new phages.
